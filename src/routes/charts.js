        ...
        if (params.debug) {
            ...
            chartFactory.createChartFor(type, data, (err, chartLocation) => {
                if (err) {
                    ...
                    s3Helper.uploadChart(path.join(__dirname, '..', '..', 'gen', chartLocation), (type === chartFactory.Type.PIE || type === chartFactory.Type.DONUT || type === chartFactory.Type.REVIEWS_SCORECARD || type === chartFactory.Type.INSTALL_SUMMARY || type === chartFactory.Type.REVENUE_SUMMARY), (err, s3Location) => {
                        if (err) {
